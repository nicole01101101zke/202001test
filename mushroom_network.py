...
def initialize_parameters(layer_dims):
	"""
	:param layer_dims: list,ÿһ�㵥Ԫ�ĸ�����ά�ȣ�
	:return:dictionary,�洢����w1,w2,...,wL,b1,...,bL
	"""
	np.random.seed(3)
	...
	for l in range(1,L):
		...
		# parameters["W" + str(l)] = np.zeros((layer_dims[l], layer_dims[l - 1])) #Ϊ�˲��Գ�ʼ��Ϊ0�ĺ���
...
def forward_propagation(X, parameters):
	...
	for l in range(1,L):
		...
		caches.append((A, W, b, z))  # �Լ����Ϊ�ָ��z��Ϊ����һ���ģ������������ֵA��Ϊ����һ�������룬���鵽��һ�㡣ע��cache��λ�ã�Ҫ����reluǰ�档
...
def compute_cost(AL,Y):
	"""
	:param AL: ����һ���ļ���ֵ����Ԥ��ֵ��shape:(1,number of examples)
	:param Y:��ʵֵ,shape:(1, number of examples)
	:return:
	"""
	m = Y.shape[1]
	# cost = -1.0/m * np.sum(Y*np.log(AL)+(1-Y)*np.log(1.0 - AL))#py��*�ǵ���
	# cost = (1. / m) * (-np.dot(Y, np.log(AL).T) - np.dot(1 - Y, np.log(1 - AL).T)) #�Ƽ��������������Ǹ����׳���
	cost = 1. / m * np.nansum(np.multiply(-np.log(AL), Y) +
	                          np.multiply(-np.log(1 - AL), 1 - Y))
	#����������״��ɾ����ά��Ŀ������shape��Ϊ1��ά��ȥ����������[[[2]]]����2
...
def relu_backward(dA, Z):
	...
	:return:
	"""
	dout = np.multiply(dA, np.int64(Z > 0)) #J��z������
	return dout
